...
class AllPositionsView(CreateAPIView,ListAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if not balancedata:
            return Response('there is no wallet for this account',status=status.HTTP_400_BAD_REQUEST)

        balance = balancedata.balance


        if len(data)<1:
            return Response("no open position for this account",status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response("all positions has been deleted",status=status.HTTP_200_OK)
